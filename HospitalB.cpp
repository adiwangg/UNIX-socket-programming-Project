...
int main(int argc, char const *argv[])
{
    ...
    char buffer[1024] = {0};
    string fileName = "../HospitalB.txt";
    string nameHospital = "Hospital B";
    string line;
    ifstream file;
...
// Convert IPv4 and IPv6 addresses from text to binary form
    ...
    if (getsockname(sock, (struct sockaddr *)&client_addr, (socklen_t*)&len) == -1)
    {
        ...
        cout << "<Hospital B> has TCP port " << ntohs(client_addr.sin_port) << " and IP address " << inet_ntoa(client_addr.sin_addr) << " for Phase 1";
        cout << endl;
